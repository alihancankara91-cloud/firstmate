...
- **Explicit delivery posture** - each project records `no-mistakes`, `direct-PR`, `local-only`, or the conditional `no-mistakes-prod-only` policy, with an optional `+yolo` autonomy flag; every ship task resolves that posture to one concrete delivery mode.
- **Optional secondmates** - opt in to persistent second mates that run from isolated firstmate homes with their own `FM_HOME`, state, projects, and session lock, supervising project clones or a project-less firstmate-repo domain, kept on the primary firstmate version by guarded local fast-forwards and checked for live agent processes at session start.
- **Event-driven, zero-token supervision** - a bash watcher sleeps on the fleet and wakes the first mate only when something needs you; Pi visibly relays queued decisions and blockers, and verified primary harnesses refuse a turn end while a home-local escalation is still undrained or work is under way without live supervision.
...
```
 ...
 └───┬────┘   └───┬────┘      └───┬────┘
  ...
  treehouse worktree, Orca worktree, or isolated secondmate home
     │
     ├─ ship: resolved task mode ► PR/local merge ► teardown
     │
...
Full architecture - the supervision engine, worktree isolation, secondmates, dispatch profiles, delivery posture, optional X mode, fleet sync, and self-update - is in [docs/architecture.md](docs/architecture.md).
...
- [docs/architecture.md](docs/architecture.md) - maintainer architecture for the crew, supervision, worktrees, secondmates, and delivery posture.
...
- [docs/codex-sandbox-backend.md](docs/codex-sandbox-backend.md) - the external macOS cage applied to every Codex worker launch.
