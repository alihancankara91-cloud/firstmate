...
case "$FM_ROOT" in
  /*) ;;
  *)
    FM_WATCH_ROOT_INPUT=$FM_ROOT
    FM_ROOT=$(CDPATH='' cd -- "$FM_WATCH_ROOT_INPUT" 2>/dev/null && pwd -P) || {
      echo "error: FM_ROOT directory cannot be resolved: $FM_WATCH_ROOT_INPUT" >&2
      exit 1
    }
    ;;
esac
