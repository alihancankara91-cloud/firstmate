...
# Usage: fm-spawn.sh <task-id> <project-dir> --mode <no-mistakes|direct-PR|local-only> --yolo <on|off> [--harness <name>|harness|launch-command] [--model <name>] [--effort <level>] [--backend <name>] [--requires <task-id>]...
#        fm-spawn.sh <task-id> <project-dir> --scout [--harness <name>|harness|launch-command] [--model <name>] [--effort <level>] [--backend <name>] [--requires <task-id>]...
...
if [ -d "$META_PATH" ] || [ -L "$META_PATH" ]; then
  echo "error: unsafe task metadata path at $META_PATH" >&2
  exit 1
fi
{
  ...
  echo "error: could not publish task metadata at $META_PATH" >&2
  exit 1
}
...
# Export the task temp roots into the crewmate's pane shell so the agent and
# every child inherit them. TMPDIR gives Puppeteer a task-attributable profile
# path; GOTMPDIR keeps Go build temp separately nested. Send both before launch.
spawn_send_text_line "$T" "export TMPDIR=$BROWSER_TMP"
