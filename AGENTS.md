...
state/               volatile runtime signals; gitignored
  ...
  <id>.delivery-override    loud recorded delivery-gate exception written by fm-delivery-gate.sh override; removed by teardown
  <id>.meta          task metadata; bin/fm-spawn.sh owns its base fields and docs/configuration.md "Runtime backend" owns backend-specific fields and selector interpretation; fm-pr-check, including through fm-pr-merge, records one canonical pr= and the forge's pr_head= when available (GitHub pull requests and GitLab merge requests; docs/gitlab-merge-watch.md); fm-x-link appends x_request=, x_request_ts=, x_followups=, and optional x_platform=/x_reply_max_chars= for an X-mode-originated task (section 14); fm-delivery-gate.sh expect appends firstmate-recorded deliverable= paths
  ...
  .claude-autoarm.lock .claude-autoarm-epoch .claude-autoarm-failure-notified .claude-autoarm-failure-alarmed .turnend-claude-blocks .turnend-claude-escalation-blocks .turnend-claude-blocks.lock   Claude Stop auto-arm single-flight, epoch, failure-episode, attended-alarm, supervision-budget, escalation-budget, and budget-lock records; never touch
...
4. **Supervision operating instructions** - after the wake queue and before context, the digest emits exactly one operating block for the detected primary harness.
...
7. **Next step** - prints the context-specific watcher next step.
...
Dispatch a dependent step with `fm-spawn`'s `--requires <predecessor-id>` so it cannot start before the predecessor's durable delivery evidence exists.
...
A commit, a pushed branch, or a green pipeline is never delivery: teardown also enforces the delivery gate, which verifies against the remote that the work is delivered through the raised PR's own file list; `bin/fm-delivery-lib.sh` owns that contract.
Operate the gate through `bin/fm-delivery-gate.sh` - verify when a worker reports done, record expected deliverable paths at intake, and use its loud recorded override as the only sanctioned exception.
