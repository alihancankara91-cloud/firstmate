...
8. The turn-end guard (`bin/fm-turnend-guard.sh --claude`) remains the final backstop.
   ...
   It allows the stop when a watcher is healthy, the role-verified auto-arm owns recovery for this event epoch, or a fresh rewake is recorded, while fresh failure epochs advance the bounded one-time attended fail-open progression described in [`turnend-guard.md`](../turnend-guard.md).
   An undrained open worker escalation still requires a bounded handling continuation.
