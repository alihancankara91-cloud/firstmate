...
The producing PR and X helpers own the fields they append, `bin/fm-classify-lib.sh` owns status-event vocabulary and the structured escalation format rendered into new briefs, and `bin/fm-crew-state.sh` owns current-state reconciliation.
...
Secondmate routes cover `no-mistakes`, `direct-PR`, and `no-mistakes-prod-only` projects; `local-only` projects remain main-firstmate work.
For `no-mistakes` and `no-mistakes-prod-only` projects, seeding initializes only projects newly cloned into a secondmate home and refuses to mutate a preexisting clone that is not already initialized.
...
Before `fm-brief.sh`, `fm-spawn.sh`, or `fm-afk-launch.sh` persists a path or passes it to another process, it resolves each applicable relative `FM_ROOT_OVERRIDE`, `FM_HOME`, `FM_STATE_OVERRIDE`, or `FM_DATA_OVERRIDE` directory against the caller's working directory, preserves absolute spellings unchanged, and rejects an unresolvable relative directory with the offending variable named.
`fm-bootstrap.sh` and the path-authenticated X-mode consumers `fm-pr-check-migrate.sh`, `fm-watch.sh`, and `fm-x-poll.sh` likewise resolve relative root, home, and state inputs before deriving or validating path-bound artifacts.
...
FM_HEARTBEAT_SCAN_SECS=300         # cadence of the catch-all scan for missed captain-relevant status events
