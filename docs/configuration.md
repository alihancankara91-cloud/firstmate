...
Every new task records `tasktmp=` for its task temp root and `browsertmp=` for the home-scoped `TMPDIR` whose path makes Puppeteer browser profiles attributable during task cleanup and periodic orphan reaping.
