...
Only `genuine-user-prompt`, `genuine-agent-response`, and `working-status` are generally policy-visible.
The native `firstmate-escalation-relay` custom message is a narrow safety exception that remains visible so an exact decision or blocker cannot disappear during its triggered handling turn; [`watcher-continuity.md`](watcher-continuity.md) owns its transport mechanics.
...
| `custom-message` | `CustomMessageComponent` when `display` is true | The session-start nudge and legacy Calm context messages use `display: false`; the native `firstmate-escalation-relay` is the deliberate visible safety exception owned by [`watcher-continuity.md`](watcher-continuity.md), and arbitrary extension messages remain an unsupported boundary. |
