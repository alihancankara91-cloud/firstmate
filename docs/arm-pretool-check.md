...
It runs only in a primary home, and it denies only an executed `bin/fm-*.sh` command other than `bin/fm-wake-drain.sh`, `bin/fm-watch-arm.sh`, `bin/fm-send.sh`, `bin/fm-agent-exit.sh`, or the ordinary literal `bin/fm-teardown.sh` when task metadata is in flight and no identity-matched live watcher holds that home's lock.
Ordinary shell commands, fleet-script names used as data, all commands in an idle fleet, child worktrees, wake drain, watcher arm, the authenticated steer channel, safe agent shutdown, and ordinary literal teardown remain allowed.
...
The separate `bin/fm-turnend-guard.sh` Stop integration remains the final backstop.
