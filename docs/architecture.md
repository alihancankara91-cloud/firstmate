...
The same backstop refuses a turn while an undrained home-local escalation remains open.
...
`bin/fm-backend.sh` centralizes backend selection, `state/<id>.meta` helpers, current-schema cleanup validation, authenticated legacy-record migration, selector resolution, and operation dispatch; `bin/backends/tmux.sh` is the verified reference adapter ([`docs/tmux-backend.md`](tmux-backend.md)), and `bin/backends/herdr.sh` (P2), `bin/backends/zellij.sh` (P3), `bin/backends/orca.sh` (P4), and `bin/backends/cmux.sh` (P5) are experimental task-spawn adapters.
...
Ship tasks change projects and ship by a concrete per-task delivery mode (`no-mistakes`, `direct-PR`, or `local-only`); scout tasks leave standalone investigation reports at `data/<id>/report.md` and never push.
...
Teardown is fail-closed for ship worktrees: dirty worktrees refuse, committed work must be landed before the worktree is returned, and - on top of that - the work must clear the delivery gate: verified remote delivery through the raised PR's own forge-side file list, or content already on the remote default branch, with a loud recorded override as the only sanctioned exception.
A gate pass or override writes the durable `data/<id>/delivered.md` evidence that `bin/fm-spawn.sh --requires` reads; a not-applicable result - nothing produced, or an unprovable local-only containment - lets teardown proceed while recording no such evidence, so dependent steps stay refused.
[`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header owns the landed-work proofs, PR-discovery fallback, and stale-lock recovery procedure; [`bin/fm-delivery-lib.sh`](../bin/fm-delivery-lib.sh)'s header owns the delivery-gate contract.
