...
The cleanup identity boundary was validated on 2026-07-29 with tmux 3.6a and metadata fixtures for every supported backend.
...
ok - legacy cleanup identity: exact registered worktrees authenticate recorded and pathless Orca cleanup
ok - secondmate endpoint identity: Herdr uses the child home while Zellij remains parent-owned
ok - legacy endpoint identity: owning home is explicit for top-level and recursive cleanup
ok - cleanup identity: live task-bound and authoritatively absent legacy endpoints migrate while crossed records refuse
...
Current task-bound records use metadata validation before backend dispatch, while authenticated legacy migration uses exact worktree provenance and owning-container inventories before atomically binding and revalidating the record.
...
The latest core CLI matrix verification uses Herdr 0.7.5 protocol 16 on macOS aarch64, with earlier 0.7.4, protocol-14, and 0.7.3 evidence retained where they define current behavior or fallbacks.
