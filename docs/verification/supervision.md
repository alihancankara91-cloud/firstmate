...
The current Stop-owned main/secondmate inclusion, child-worktree exclusion, and dash-leading macOS login-shell `comm` handling in both shared session-lock identity predicates are covered deterministically by `tests/fm-claude-stop-autoarm.test.sh`.
...
Session-lock ownership is decided against a session's whole contiguous harness ancestry rather than one chosen pid, so the Stop auto-arm reaches its lock owner wherever that owner sits: the outermost pid of Claude Code's multi-level `bg-spare` hook worker chain, or an inner pid when a harness-named daemon parents the session.
