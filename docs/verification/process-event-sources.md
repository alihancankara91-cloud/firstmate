...
| adapter-owned terminal verdict | two fixture adapters - one that ends on any result, one with no terminal knowledge - decide the outcome alone: the first has its registration retired and terminal cutoff preserved automatically after one capture and is never restarted, the second stays armed |
...
| registration-generation retirement | an old terminal runner preserves a concurrently replaced registration so the replacement runs independently; a terminal cutoff retires registrations already present in another home while allowing a later registration to advance; injected registration-removal failure retains terminal state, performs no second poll, and completes idempotently once removal recovers |
| legacy registration retirement | a registration with neither a generation field nor a machine-wide counter retires as generation zero after one terminal capture and never restarts |
...
| terminal tombstone teardown | teardown applies the runner's validated terminal classification, removes the former secondmate home, and preserves the machine-wide tombstone |
