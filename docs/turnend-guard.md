...
The guard also blocks while the durable home-local wake queue maps to a still-open, undrained `needs-decision` or `blocked` event, even when watcher supervision is healthy.
...
The passive-adapter preambles preserve the shared guard's concrete reason because a refusal can identify missing supervision, a pending escalation, or both.
...
`tests/fm-turnend-guard.test.sh` covers the predicate, main and secondmate primary scope, child-worktree exclusion, `FM_HOME` and `FM_STATE_OVERRIDE` precedence, pending structured and legacy escalations, drain and resolution handling, cross-home exclusions, the cooperative `--claude` claim wait, epoch allow, re-block budget, Pi logical-run latching, missing-`jq` behavior, all five primary registrations, Grok native and legacy selection, typed field precedence, malformed input, and exactly-one-path safety.
