...
test_kimi_launch_then_send_is_verified() {
  ...
  # fm-spawn records a home-scoped task temp root (/tmp/fm-$HOME_TAG/$ID); read the
  # recorded value rather than assuming a flat /tmp/fm-$ID path.
  task_tmp=$(grep -E '^tasktmp=' "$meta" | cut -d= -f2-)
  KIMI_RUNTIME_TASK_TMP=$task_tmp
  [ -n "$task_tmp" ] || fail "kimi meta did not record its task temp root"
