...
test_brief_assertion_precedes_branch() {
  ...
  assert_grep "blocked [key=isolation]: need=launched in the primary checkout instead of an isolated worktree | action=launch this task in a disposable isolated worktree | recommend=stop here and relaunch through the standard spawn path" "$brief" \
  ...
  iso=$(grep -n 'blocked \[key=isolation\]:' "$brief" | head -1 | cut -d: -f1)
