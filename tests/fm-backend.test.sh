...
# fm-delivery-lib.sh is a fork sibling because teardown sources the delivery gate.
# fm-public-followup-lib.sh, fm-secondmate-registry-lib.sh, and fm-x-lib.sh are
# upstream siblings teardown now requires for public-followup and registry paths.
OLD_BIN_UNCHANGED_SIBLINGS="fm-gate-refuse-lib.sh fm-guard.sh fm-lock-lib.sh fm-tasks-axi-lib.sh fm-pr-lib.sh fm-delivery-lib.sh fm-tangle-lib.sh fm-tmux-lib.sh fm-composer-lib.sh fm-wake-lib.sh fm-procevent-lib.sh fm-classify-lib.sh fm-supervision-lib.sh fm-ff-lib.sh fm-config-inherit-lib.sh fm-project-mode.sh fm-harness.sh fm-crew-state.sh fm-decision-hold.sh fm-backend.sh fm-operational-input.sh fm-public-followup-lib.sh fm-secondmate-registry-lib.sh fm-x-lib.sh"
...
set -u
{ printf 'tmux'; for a in "$@"; do printf '\x1f%s' "$a"; done; printf '\n'; } >> "${FM_TMUX_LOG:?}"
# The strict existence probe (list-panes) reports the window gone so the new
# teardown's verified endpoint removal sees a clean first kill.
[ "${1:-}" != list-panes ] || exit 1
exit 0
SH
  cat > "$fb/treehouse" <<'SH'
...
test_teardown_conformance_old_vs_new() {
  ...
  # Verified endpoint removal adds a strict list-panes probe after the kill.
  # Exclude that deliberate post-baseline behavior from byte parity while
  # asserting it independently below.
  grep -v $'^tmux\x1flist-panes' "$log_old" | tr -d '=' > "$TMP_ROOT/teardown-old-normalized.log" || true
  grep -v $'^tmux\x1flist-panes' "$log_new" | tr -d '=' > "$TMP_ROOT/teardown-new-normalized.log" || true
