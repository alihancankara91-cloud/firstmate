...
# fm-operational-input.sh is the same kind of copied sibling: fm-marker-lib.sh
# is a compatibility entry point that sources it unconditionally, so the old
# bin/ needs a real copy or `. "$_FM_MARKER_LIB_DIR/fm-operational-input.sh"`
# aborts the whole old-side run under set -eu.
...
test_teardown_conformance_old_vs_new() {
  ...
  # The verified endpoint removal (kill_endpoint_verified) adds a strict
