...
SH
  ...
  got=$(PATH="$fakebin:$BASE_PATH" PI_CODING_AGENT='' FM_PI_HARNESS=pi-signed "$ROOT/bin/fm-harness.sh")
...
test_spawn_explicit_harness_does_not_inherit_secondmate_harness_tokens() {
  ...
  launch=$(cat "$launchlog")
  assert_contains "$launch" "fm-codex-cage.sh' --worktree" \
    "explicit-harness-no-tokens: secondmate codex launch did not use the external cage"
  assert_contains "$launch" "-- codex --dangerously-bypass-approvals-and-sandbox" \
    "explicit-harness-no-tokens: launch did not use bypass-mode codex inside the cage"
  assert_not_contains "$launch" "--sandbox workspace-write" \
    "explicit-harness-no-tokens: launch retained the interim workspace-write posture"
